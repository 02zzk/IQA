...
class SCAModule(nn.Module):
    ...
    def forward(self, x):
        ...
        avg_pool = avg_pool.view(avg_pool.size(0), -1)  # [B, C]
        
        # 计算通道权重
        x = F.relu(self.fc1(avg_pool))
        x = self.fc2(x)
        attention = torch.sigmoid(x).view(x.size(0), x.size(1), 1, 1)
...
class SVTSCNet(nn.Module):
    ...
    def forward_cross_attention(self, x,y=None,brightness=None, angle=None):
        ...
        for i, (feat, svt_module) in enumerate(zip(dist_feat_list, self.svt_modules)):
            ...
            feat_weighted = self.sca_modules[i](feat_combined).to(device)
            conv = self.convs[i]
