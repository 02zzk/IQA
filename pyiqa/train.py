...
def create_train_val_dataloader(opt, logger):
    ...
    for phase, dataset_opt in opt['datasets'].items():
        if phase == 'train':
            ...
            #     print("Brightness:", train_data['brightness'])
            #     print("Angle:", train_data['angle'])
            #     break
...
def load_resume_state(opt):
    ...
    if resume_state_path is None:
        ...
        # 处理 'iter' 键缺失的情况
        iter_start = resume_state.get('iter', 0)  # 如果没有 'iter' 键，默认为 0
        resume_state['iter'] = iter_start  # 确保返回的 resume_state 中有 'iter' 键
