...
class waferIQAModel(BaseModel):
    """General module to train an IQA network."""

    def __init__(self, opt):
        super(waferIQAModel, self).__init__(opt)
    ...
    def feed_data(self, data):
        ...
        #print(f"img_input: {self.img_input}")  # 打印 MOS 标签
        if 'mos_label' in data:
            self.gt_mos = data['mos_label'].to(self.device)
            #print(f"MOS Label: {self.gt_mos}")  # 打印 MOS 标签
        if 'ref_img' in data:
            self.use_ref = True
        ...
        if 'brightness' in data:
            self.brightness = data['brightness'].unsqueeze(1).to(self.device).to(torch.float32)  # 转换为 (N, 1)
            #print(f"Brightness: {self.brightness}")  # 打印 brightness

            # Process the angle value if available
        if 'angle' in data:
            self.angle = data['angle'].unsqueeze(1).to(self.device).to(torch.float32)  # 转换为 (N, 1)
            #print(f"Angle: {self.angle}")  # 打印 angle

    def net_forward(self, net):
        if self.use_ref:
            return net(self.img_input, self.ref_input,self.brightness,self.angle)
        else:
            return net(self.img_input,None,self.brightness,self.angle)
            #return net(self.img_input, None)
